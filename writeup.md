...
And one more rotation needed to correct axis orientation from link 6 to the world frame (known as R_corr() in the lessons):
...
Once theta1, theta2, and theta3 are computed, FK is used to compute a rotation matrix for links 0 to 3.  Using roll, pitch, and yaw values we can also compute the gripper rotation. These two are then used to compute a gripper rotation matrix relative to link3.  This is a little tricky due do the fact that the axis orientations flip between link3 and the world frame so axis flips are also adjusted in the process to produce a rotation matrix that only tells us how the gripper is rotated relative to link3 of the arm.
...
For any given IK problem defined by gripper location and pose, there are many different arm configurations that can move the gripper to the correct location and pose.  The base can be rotated towards, or away from the wrist center. and links 2 and 3, can form a triangle with the elbow raised, or lowered -- creating 4 possible configurations for theta1, theta2, and theta3 to position the arm at the correct wrist center.

The test exercise will sometimes produce paths that raise the arm up and over the base to make it reach backwards and then lower it again.  Without adjusting for alternate base configurations this path will force the base to do a 180 deg flip in the middle of the motion which is slow, time consuming, and forces a momentary wild off-path swing of the gripper as the base flips around.  I wrote code to consider the alternate base configuration where joint2 can be allowed to be on the far side of the base away from the wrist center and it worked fine to stop the initial base flip motion.  However, the paths generated often then caused the based to twist around and force the arm to reach backwards to the shelf -- which was not possible because the joints in the arms could not bend far enough to reach the shelf backwards like that.  So the base rotate was required anyway.  The more advanced code only changed where the base rotated, and was not able to produce paths without base rotations due to the types of paths generated in this exercise.  So I disabled the base flip code in the final version, though the code is still in the source file.

The same alternate arm configuration problem of the base, exists for the wrist as well. But it's much worse for the wrist due to the large rotation range of joint 4 and joint 6 (+- 350 degs).  There are often as many as 6 or 8 different wrist configurations that can put the gripper into the correct orientation.
...
I wrote code to test all possible wrist configurations and pick the best one based on minimizing rotations from the previous orientation.  Unlike the base flip code that didn't help improve the base flip problem in the paths of this exercise, the wrist orientation code greatly reduced the need for wrist flips in the paths.  They almost never happen now for the paths of this exercise. The gripper is almost always able to move through the motions without need for wrist flips in the middle of the motion.  At the end of the pick and place, when the arm moves back to the start, it now has to "unwind" twists created by the pick and place motions. So you will notice a set of smooth roatations and flips to get the arm back to the zero starting position as it moves back from the bin, to the starting location.
...
The hack above represents a protocol weakness of the IK request. It could be improved by including information about the starting configuration of the arm but doesn't now, so the hack above was needed to create optimal IK solutions that didn't add an extra unneeded wrist flip at the start of the place cycle.
